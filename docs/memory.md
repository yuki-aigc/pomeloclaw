...
> 本文档说明 srebot 的记忆机制、检索机制、会话机制与运维要点。

参考：
- [智能体 Memory](https://docs.智能体.ai/zh-CN/concepts/memory)
...
- 采用硬性上限控制注入体积（按文件/总量裁剪），思路与 智能体 的 `maxInjectedChars` 限额一致。
...
1. 记忆来源与权威层
- 智能体：文档强调 Memory 文件（如 `MEMORY.md`）与可扩展记忆源，检索基于索引，不是每次全量读 md。
- srebot：也是“文件 + 索引”模式，但额外引入了 `dingtalk_sessions` / `session_events` 的会话态数据库层。

2. 会话记忆能力
- 智能体：文档提到有 experimental session memory 方向。
- srebot：DingTalk 侧已经把 session events 纳入统一检索入口，实用性更强。

3. 工具面
- 智能体：文档强调 `memory_search` / `memory_get` / `memory_save` 组合。
- srebot：已补齐 `memory_search` + `memory_get` + `memory_save` 三件套，并额外支持 session events 路径精读。

4. 同步策略
- 智能体：强调后台 watcher + debounce 的异步索引同步。
- srebot：采用“启动基线 + 事件触发 + 检索前条件同步”的组合策略，更偏工程可控。
