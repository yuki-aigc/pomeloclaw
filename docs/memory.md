...
- `target=daily` -> `workspace/memory/YYYY-MM-DD.md`
- `target=long-term` -> `workspace/MEMORY.md`
...
- 当 `shared_main_scope_reads=true` 时，隔离 scope 在检索阶段会额外读取 `main` 的团队记忆文件（`MEMORY.md`、`memory/YYYY-MM-DD.md`、主 `HEARTBEAT.md`）。
